...
### Lokaler `secure-ingress`-Caddy-Provider
...
Der lokale Provider ist mit Caddy `2.11.4` fest versioniert. Die nicht geheime Beispielkonfiguration liegt unter [`deploy/secure-ingress/caddy/provider.example.toml`](deploy/secure-ingress/caddy/provider.example.toml). Sie kann read-only geplant werden:

```bash
python3 scripts/ralf-secure-ingress-caddy.py plan \
  --config deploy/secure-ingress/caddy/provider.example.toml
```

Eine geschützte Caddyfile wird mit einem zuvor versionsgeprüften Caddy-Binary erzeugt und von Caddy validiert; das Kennwort wird standardmäßig verdeckt abgefragt:

```bash
python3 scripts/ralf-secure-ingress-caddy.py render \
  --config deploy/secure-ingress/caddy/provider.example.toml \
  --caddy /pfad/zu/caddy \
  --output /vorhandenes/verzeichnis/Caddyfile
```

Die Ausgabe besitzt Modus `0600` und enthält den Argon2id-Kennworthash; sie ist deshalb als sensibles Deploymentartefakt zu schützen und nicht zu committen. Der isolierte lokale Test lädt das fest gepinnte offizielle Release ausschließlich temporär und prüft TLS, CA, CIDR-Allowlist, Authentifizierung, Proxying und Headerbereinigung:

```bash
./tests/secure-ingress-caddy.sh
```

In VMID 100 ist noch kein LAN-Ingress installiert oder aktiviert. Es gibt weiterhin keine LAN-Freigabe, keine mutierenden Setup-Formulare und keine zusätzlichen Netzwerk-Capabilities. Flask vertraut dem Proxyvertrag noch nicht produktiv; `TRUSTED_HOSTS`, das Vertrauen in exakt einen Proxy-Hop und der reproduzierbare Deploymentpfad folgen getrennt. Vorhandene externe Reverse Proxies sollen später über weitere Provider eingebunden werden, ohne Gunicorn direkt im LAN zu öffnen.
