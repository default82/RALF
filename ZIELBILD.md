...
| M-029 | AKTIV | Implementiere das erste read-only Grundgerüst des dauerhaften Bootstrap- und Statusdienstes mit Python-Projekt, Flask-Anwendung, lokaler HTML-Darstellung, Statusermittlung, `GET /`, `GET /healthz` und `GET /api/v1/status` sowie Tests und installierbarem Paket. SQLite-Schreibvorgänge, Setup-Fragen, Installationsplanung, mutierende Aktionen, LAN-Freigabe, Authentifizierung, Modellzugriff, Ollama, Open WebUI, MCP und Adapterverwaltung gehören nicht zu diesem Schritt. |
