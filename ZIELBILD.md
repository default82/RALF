...
| M-035 | AKTIV | Der real installierte Statusdienst muss ohne Laufzeitfehler starten und den lokalen Netzwerkzustand korrekt erfassen. In VMID 100 ist der Dienst erreichbar, Gunicorn 26 protokolliert jedoch einen nichtfatalen Control-Socket-Schreibfehler unter `/nonexistent`, und die systemd-Adressfamilienbeschränkung ohne `AF_NETLINK` verhindert die `ip`-Statusabfragen. Beide Abweichungen sind vor weiteren Funktionsausbauten reproduzierbar und minimal zu beheben. |
