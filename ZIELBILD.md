...
| M-035 | AKTIV | Die minimalen Repositorykorrekturen sind implementiert und lokal getestet: Der nicht benötigte Gunicorn-Control-Socket wird mit `--no-control-socket` deaktiviert, und `AF_NETLINK` wird ausschließlich für die read-only `ip`-Abfragen des Netzwerkstatus freigegeben. Die übrige Härtung und die leeren Capability-Grenzen bleiben unverändert. Die reale Unit in VMID 100 ist noch nicht aktualisiert und muss über einen separat geplanten und freigegebenen Unit-Update-Pfad validiert werden. |
| M-036 | ABGESCHLOSSEN | Für bereits vollständig installierte Bootstrap-Versionen existiert ein eng begrenzter Plan-/Apply-Unit-Update-Pfad. Er akzeptiert ausschließlich die bekannte alte Unit mit SHA-256 `8f5b30c7d9335824dfabb19cab5b338337860a45e785a6985370da9b8f6f48d7` oder die geprüfte aktuelle Unit, überträgt höchstens Ziel-Unit, Gast-Updater und Manifest, ersetzt keine Anwendungskomponenten und führt höchstens einen `daemon-reload` sowie einen Dienstrestart aus. Die reale Ausführung in VMID 100 bleibt separat freigabepflichtig. |
