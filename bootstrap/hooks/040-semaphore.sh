...
log() { printf '[hook:040-semaphore] %s\n' "$*"; }
warn() { printf '[hook:040-semaphore][warn] %s\n' "$*" >&2; }
...
vmid="10010"
hostname="semaphore-svc"
ip_cidr="10.10.100.10/16"

gateway="${RALF_GATEWAY:-10.10.0.1}"
bridge="${RALF_BRIDGE:-vmbr0}"
cores="${SEMAPHORE_CORES:-2}"
mem_mb="${SEMAPHORE_MEM_MB:-2048}"
disk_gb="${SEMAPHORE_DISK_GB:-32}"
...
if [[ "$mode" == "plan" ]]; then
  log "PLAN: würde Semaphore LXC erstellen/abgleichen (CTID=$vmid, IP=$ip_cidr)."
  ...
  init_semaphore_service "$vmid" "${RALF_DOMAIN:-otta.zone}"
  log "APPLY: Semaphore LXC bereitgestellt."
fi

echo "semaphore_ctid=$vmid" > "$runtime_dir/state/semaphore.state"
echo "semaphore_ip=$ip_cidr" >> "$runtime_dir/state/semaphore.state"
