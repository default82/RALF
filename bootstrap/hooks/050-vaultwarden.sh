...
log() { printf '[hook:050-vaultwarden] %s\n' "$*"; }
warn() { printf '[hook:050-vaultwarden][warn] %s\n' "$*" >&2; }
...
vmid="5010"
hostname="vaultwarden-svc"
ip_cidr="10.10.50.10/16"

gateway="${RALF_GATEWAY:-10.10.0.1}"
bridge="${RALF_BRIDGE:-vmbr0}"
cores="${VAULTWARDEN_CORES:-1}"
mem_mb="${VAULTWARDEN_MEM_MB:-512}"
disk_gb="${VAULTWARDEN_DISK_GB:-8}"
...
if [[ "$mode" == "plan" ]]; then
  log "PLAN: würde Vaultwarden LXC erstellen/abgleichen (CTID=$vmid, IP=$ip_cidr)."
  ...
  init_vaultwarden_service "$vmid" "${RALF_DOMAIN:-otta.zone}"
  log "APPLY: Vaultwarden LXC bereitgestellt."
fi

echo "vaultwarden_ctid=$vmid" > "$runtime_dir/state/vaultwarden.state"
echo "vaultwarden_ip=$ip_cidr" >> "$runtime_dir/state/vaultwarden.state"