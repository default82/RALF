...
log() { printf '[hook:060-prometheus] %s\n' "$*"; }
warn() { printf '[hook:060-prometheus][warn] %s\n' "$*" >&2; }
...
cores="${PROMETHEUS_CORES:-2}"
mem_mb="${PROMETHEUS_MEM_MB:-2048}"
disk_gb="${PROMETHEUS_DISK_GB:-32}"
...
if [[ "$mode" == "plan" ]]; then
  log "PLAN: würde Prometheus LXC erstellen/abgleichen (CTID=$vmid, IP=$ip_cidr)."
  ...
  init_prometheus_service "$vmid"
  log "APPLY: Prometheus LXC bereitgestellt."
fi

echo "prometheus_ctid=$vmid" > "$runtime_dir/state/prometheus.state"
echo "prometheus_ip=$ip_cidr" >> "$runtime_dir/state/prometheus.state"