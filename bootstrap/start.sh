...
# Load MinIO creds for dependent settings
set -a
...
if [[ ! -f \"\$POSTGRES_ENV\" ]]; then
  ...
  MAIL_DB_USER=\"\${MAIL_DB_USER:-mail}\"
  MAIL_DB_PASSWORD=\"\${MAIL_DB_PASSWORD:-\$(rand)}\"
  MAIL_DB_NAME=\"\${MAIL_DB_NAME:-mail}\"
  N8N_DB_USER=\"\${N8N_DB_USER:-n8n}\"
  N8N_DB_PASSWORD=\"\${N8N_DB_PASSWORD:-\$(rand)}\"
  N8N_DB_NAME=\"\${N8N_DB_NAME:-n8n}\"
  GITEA_DB_USER=\"\${GITEA_DB_USER:-gitea}\"
  GITEA_DB_PASSWORD=\"\${GITEA_DB_PASSWORD:-\$(rand)}\"
  GITEA_DB_NAME=\"\${GITEA_DB_NAME:-gitea}\"
  VAULTWARDEN_DB_USER=\"\${VAULTWARDEN_DB_USER:-vaultwarden}\"
  VAULTWARDEN_DB_PASSWORD=\"\${VAULTWARDEN_DB_PASSWORD:-\$(rand)}\"
  VAULTWARDEN_DB_NAME=\"\${VAULTWARDEN_DB_NAME:-vaultwarden}\"
...
# Tofu/Terragrunt remote state (MinIO/S3)
...
if [[ ! -f \"\$TFSTATE_ENV\" ]]; then
  ...
  cat >\"\$TFSTATE_ENV\" <<EOF
TFSTATE_ENABLE_REMOTE=\$TFSTATE_ENABLE_REMOTE
TFSTATE_S3_BUCKET=\$TFSTATE_S3_BUCKET
TFSTATE_S3_PREFIX=\$TFSTATE_S3_PREFIX
TFSTATE_S3_REGION=\$TFSTATE_S3_REGION
TFSTATE_S3_ENDPOINT=\$TFSTATE_S3_ENDPOINT
...
if [[ ! -f \"\$SYNAPSE_ENV\" ]]; then
  ...
  SYNAPSE_ADMIN_PASSWORD_RALF=\"\${SYNAPSE_ADMIN_PASSWORD_RALF:-\$(rand)}\"
  SYNAPSE_ADMIN_PASSWORD_KOLJA=\"\${SYNAPSE_ADMIN_PASSWORD_KOLJA:-\$(rand)}\"
  SYNAPSE_SMTP_HOST=\"\${SYNAPSE_SMTP_HOST:-10.10.110.20}\"
...
SYNAPSE_ADMIN_PASSWORD_RALF=\$SYNAPSE_ADMIN_PASSWORD_RALF
SYNAPSE_ADMIN_PASSWORD_KOLJA=\$SYNAPSE_ADMIN_PASSWORD_KOLJA
SYNAPSE_SMTP_HOST=\$SYNAPSE_SMTP_HOST
SYNAPSE_SMTP_PORT=\$SYNAPSE_SMTP_PORT
SYNAPSE_SMTP_USER=\$SYNAPSE_SMTP_USER
SYNAPSE_SMTP_PASSWORD=\$SYNAPSE_SMTP_PASSWORD
...
if [[ ! -f \"\$VAULTWARDEN_ENV\" ]]; then
  ...
  VAULTWARDEN_DB_PORT=\"\${VAULTWARDEN_DB_PORT:-5432}\"
...
if [[ ! -s \"\$N8N_ENV\" ]]; then
  ...
  N8N_ENCRYPTION_KEY=\"\${N8N_ENCRYPTION_KEY:-\$(rand)}\"
  N8N_BASIC_AUTH_USER=\"\${N8N_BASIC_AUTH_USER:-ralf}\"
...
GITEA_MIRROR_SOURCE_URL=\$GITEA_MIRROR_SOURCE_URL
EOF
fi
