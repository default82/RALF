...
Die folgende Darstellung ist Mermaid‑ready und bildet sowohl Ebenen als auch die wichtigsten gerichteten Informationsflüsse ab. Sie ist so geschrieben, dass sie ohne weitere Interpretation als Diagramm gerendert werden kann.
...
flowchart TB
  ...
  subgraph L1[Ebene 1 – Physische Infrastruktur (Robotic)]
    HW[Server/Hosts]
    SW[Managed Switches]
    PWR[Strom/USV/Topologie]
  end


  subgraph L2[Ebene 2 – Netzwerk & Transport (Robotic)]
    FW[OPNsense Firewall/Router]
    VLAN[VLANs/Segmentierung]
    IP[Adressierung & Routing]
    EDGE[Edge/Internet-Uplink]
  end


  subgraph L3[Ebene 3 – Virtualisierung & Laufzeit (Robotic)]
    PVE[Proxmox VE]
    LXC[LXC-Container]
    VM[VMs (Ausnahmen)]
  end
  ...
  subgraph L4[Ebene 4 – Dienste & Workloads (Algorithmic)]
    SVC[Applikationen & Infrastruktur-Services]
    DEP[Service-Abhängigkeiten]
  end


  subgraph L5[Ebene 5 – Deklaratives Referenzmodell (Algorithmic)]
    YAML[Source of Truth: YAML/Inventar/Policies]
    IAC[Ansible/OpenTofu]
    SEM[Semaphore UI (Ausführung/Audit)]
  end


  subgraph L6[Ebene 6 – Beobachtungs- & Datenebene (Logic)]
    MET[Metrics]
    LOG[Logs]
    EVT[Ereignisse/Traces]
    TLINE[Verläufe/Chronologie]
  end


  subgraph L7[Ebene 7 – Experimentelle Kontroll- & Lernschicht (Logic)]
    RALF[RALF: Soll/Ist-Vergleich • Hypothesen • Experimente]
    HYP[Hypothesen & Bewertung]
    EXP[Experimentplanung]
  end


  %% =========================
  %% Strukturelle Abhängigkeiten (Ausführungspfad)
  %% =========================
  HW --> FW
  SW --> VLAN
  PWR --> HW
  ...
  %% =========================
  %% Soll-Modell und Umsetzung (Top-down)
  %% =========================
  YAML --> IAC --> SEM
  SEM --> PVE
  SEM --> LXC
...
Hinweis: Die Klammerzuordnung Robotic/Algorithmic/Logic entspricht der konzeptionellen Spiegelung des Akronyms Robotic Algorithmic Logic Framework. Das Framework ist die Gesamtheit der Ebenen und ihrer Schnittstellen.
...
Dieses Dokument stellt den aktuellen konsolidierten Wissensstand dar und ist explizit als lebendes Dokument zur weiteren Verfeinerung gedacht.
